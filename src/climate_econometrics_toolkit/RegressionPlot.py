...
class RegressionPlot():
    ...
    def build_axes(self, reg_result):
        num_plots = len([val for val in reg_result.index if val != "const" and not val.startswith("fe") and not val.startswith("ie")])
        fig, axes = plt.subplots(1,num_plots,figsize=(5,5))
        axis_count = 0
        for index in range(len(reg_result.index)):
            coef_name = reg_result.index[index]
            if coef_name != "const" and not coef_name.startswith("fe") and not coef_name.startswith("ie"):
                if num_plots == 1:
