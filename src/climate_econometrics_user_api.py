from climate_econometrics_toolkit import climate_econometrics_interface_api as api
...
from climate_econometrics_toolkit import climate_econometrics_regression as regression
...
def evaluate_model():
    if model_checks():
        model_id, _, return_string = api.run_model_analysis(copy.deepcopy(model.dataset), model)
...
def get_best_model(metric="out_sample_mse_reduction"):
    valid_metrics = ["out_sample_mse_reduction","out_sample_mse","out_sample_pred_int_cov"]
    if metric not in valid_metrics:
        print(f"Metric must be one of {valid_metrics}")
    else:
        if metric == "out_sample_mse":
...
def run_bayesian_regression(model_id):
    regression.run_bayesian_regression(model_id)
