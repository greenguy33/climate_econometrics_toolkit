...
dewpoint_temp <- function(temp, rel_hum) {
	humidity.to.dewpoint(t = temp, rh = rel_hum, temperature.metric = "celsius")
}
...
for (year in 1948:2024) {
	...
	dewpoint_raster = terra::lapp(x=terra::sds(temp_raster_c, rel_hum_raster), fun = dewpoint_temp)

	dp_extracted = exact_extract(dewpoint_raster, country_shapes, fun = "mean")
	data = c()
	data$ISO3 <- country_shapes$GMI_CNTRY
	data$dewpoint <- dp_extracted
	write.csv(data, str_interp("dewpoint/dewpoint.${year}.unweighted.csv"))

	index = 1
	for (weight in weight_vec) {
		dp_extracted = exact_extract(dewpoint_raster, country_shapes, fun = "weighted_mean", weights=weight)
		data = c()
		data$ISO3 <- country_shapes$GMI_CNTRY
		data$dewpoint <- dp_extracted
		weight_vec_name = weight_vec_names[index]
		write.csv(data, str_interp("dewpoint/dewpoint.${year}.${weight_vec_name}.csv"))
		index = index + 1
	}
}
